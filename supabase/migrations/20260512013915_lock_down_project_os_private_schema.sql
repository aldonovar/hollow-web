alter function public.create_project_snapshot_with_limit(uuid, text, jsonb) security definer;
alter function public.register_project_asset_with_limit(text, text, uuid, uuid, text, bigint, numeric, text, integer, text, jsonb) security definer;
alter function public.create_render_job_with_limit(uuid, text, text, integer, integer, jsonb) security definer;
alter function public.update_render_job_status_with_scope(uuid, text, uuid, text) security definer;
alter function public.get_project_os_usage(uuid, date) security definer;

revoke usage on schema hollow_private from public, anon, authenticated;
grant usage on schema hollow_private to service_role;
...
grant execute on function hollow_private.project_os_user_id() to service_role;
grant execute on function hollow_private.project_os_tier_for_user(uuid) to service_role;
grant execute on function hollow_private.project_os_storage_limit_bytes(text) to service_role;
grant execute on function hollow_private.project_os_render_limit_minutes(text) to service_role;
grant execute on function hollow_private.project_os_ensure_workspace_editor(uuid, uuid) to service_role;
grant execute on function hollow_private.project_os_project_workspace(uuid, uuid) to service_role;
grant execute on function hollow_private.project_os_record_usage(uuid, uuid, text, numeric, text, jsonb, date) to service_role;
grant execute on function hollow_private.create_project_snapshot_with_limit(uuid, text, jsonb) to service_role;
grant execute on function hollow_private.register_project_asset_with_limit(text, text, uuid, uuid, text, bigint, numeric, text, integer, text, jsonb) to service_role;
grant execute on function hollow_private.create_render_job_with_limit(uuid, text, text, integer, integer, jsonb) to service_role;
grant execute on function hollow_private.update_render_job_status_with_scope(uuid, text, uuid, text) to service_role;
grant execute on function hollow_private.get_project_os_usage(uuid, date) to service_role;
